// Part of this code is taken from https://github.com/radix-ui/primitives/blob/main/packages/react/use-controllable-state/src/useControllableState.tsx
...
export function useControllableState<T>(props: UseControllableStateProps<T>): UseControllableStateReturn<T> {
  const { prop, defaultProp, onChange = noop, shouldUpdate = isShouldUpdate } = props;

  const onChangeProp = useCallbackRef(onChange);
  const shouldUpdateProp = useCallbackRef(shouldUpdate);

  const [uncontrolledProp, setUncontrolledProp] = React.useState(
    defaultProp as T,
  );
